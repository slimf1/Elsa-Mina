...
[NamedCommand("removepoints")]
public class RemovePointsCommand : Command
{
    private readonly IBotDbContextFactory _dbContextFactory;
    private readonly ITemplatesManager _templatesManager;

    public RemovePointsCommand(IBotDbContextFactory dbContextFactory, ITemplatesManager templatesManager)
    {
    ...
    public override async Task RunAsync(IContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (userPoints == null)
        {
            context.ReplyLocalizedMessage("remove_points_user_not_found", user);
            return;
        }

        userPoints.Points = Math.Max(0, userPoints.Points - points);
